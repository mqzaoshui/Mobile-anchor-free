...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--save_dir', type=str, default='runs-v6-t/val/', help='evaluation save dir')
    ...
    parser.add_argument('--preprocess_img', default=False, action='store_true', help='whether to reproduce 640 infer result, overwrite some config')
    ...
    parser.add_argument('--save2txt',default=False,action='store_true',help='whether to save map etc. into txt') #xyh 2022
    args = parser.parse_args()

    # load params for reproduce 640 eval result
    if args.preprocess_img:
...
def run(data,
    ...
    val = Evaler(data, batch_size, img_size, conf_thres, \
                ...
                verbose, save2txt, do_coco_metric, do_pr_metric, plot_curve, plot_confusion_matrix)
    operate = False if not preprocess_img else True
    model = val.init_model(model, weights, task, operate)
