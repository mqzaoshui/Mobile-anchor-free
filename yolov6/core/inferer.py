...
class Inferer:
    def __init__(self, source, weights, device, yaml, img_size, half, vit):
        ...
        self.vit = vit   # to contorl the mobilevit model
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            img, img_src = self.precess_image(img_src, self.img_size, self.stride, self.half, self.vit)
    ...
    @staticmethod
    def precess_image(img_src, img_size, stride, half, vit):
        '''Process image before image inference.'''
        image = letterbox(img_src, img_size, stride=stride)[0] if not vit else letterbox(img_src, img_size, stride=stride,auto=False,return_int=True)[0]
        # Convert
