...
class Evaler:
    ...
    def init_model(self, model, weights, task, operate):
        if task != 'train':
            ...
            LOGGER.info("Model Summary: {}".format(get_model_info(model, self.img_size, operate=operate)))
    ...
    #xyh save verbose as a txt
    def save_as_txt(self, line, file):
        ...
        if task != 'speed' and len(pred_results):
            ...
            if self.verbose:
                ...
                for nc_i in range(model.nc):
                    ...
                    #xyh
                    if self.save2txt:
                        line = [model.names[nc_i], len(label_count_dicts[nc_i]["images"]), label_count_dicts[nc_i]["anns"], r[i], map50, map]
                        data = str(line)
                        save_file = os.path.join(self.save_dir,'verbose.txt')
                        self.save_as_txt(data,save_file)
            cocoEval.summarize()
