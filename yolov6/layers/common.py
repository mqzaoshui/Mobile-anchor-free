...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = rearrange(out, 'b p h n d -> b p n (h d)')  #rebulid
...
class MobileViTBlock(nn.Module):
    def __init__(self, dim, depth, channel, kernel_size, patch_size, mlp_dim, dropout=0.):
        ...
        self.conv1 = self.Conv_BN_ReLU(channel, channel, kernel_size)
        self.conv2 = self.conv_1x1_bn(channel, dim)

        self.transformer = Transformer(dim, depth, 1, 32, mlp_dim, dropout)

        self.conv3 = self.conv_1x1_bn(dim, channel)
        self.conv4 = self.Conv_BN_ReLU(2 * channel, channel, kernel_size)
